...
if(isset($_GET['album_id']) and isset($_GET['owned'])){
    ...
    if(isset($_SESSION['userLoggedIn'])){
        ...
        if($owned){
            //remove album
            $remove_endpoint = $base_url . "user/deleteUser/deleteUserOwned.php?user_id=$logged_in_user_id&album_id=$album_id";
            $remove_resource = file_get_contents($remove_endpoint);
            $remove_data = json_decode($remove_resource, true);
        } else {
            //add album
            $add_endpoint = $base_url . "user/addUser/addUserOwned.php?user_id=$logged_in_user_id&album_id=$album_id";
