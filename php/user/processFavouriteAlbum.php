...
if(isset($_GET['album_id']) and isset($_GET['favourited'])){
    ...
    if(isset($_SESSION['userLoggedIn'])){
        ...
        if($favourited){
            //remove album
            $remove_endpoint = $base_url . "user/deleteUserFavourite.php?user_id=$logged_in_user_id&album_id=$album_id";
            $remove_resource = file_get_contents($remove_endpoint);
            $remove_data = json_decode($remove_resource, true);
        } else {
            //add album
            $add_endpoint = $base_url . "user/addUserFavourite.php?user_id=$logged_in_user_id&album_id=$album_id";
